...
public class MPR {
	...
	public void abstractionProcedure(TreeComponent tc) throws Exception {
		...
		for (int i = (int) c.getMinPeriod(); i <= (int) c
			...
			/*beta*/double alpha=rm.exec2-rm.cpus*(int)(Math.floor(rm.exec2/rm.cpus)); //Transfer the interface to m' interface tasks. In Defintion 2 in 09rtsj-virtual-clustering paper
			/*alpha*/int k=(int) Math.floor(alpha);
			double exec=0;
			Task t;
			int num=0;
